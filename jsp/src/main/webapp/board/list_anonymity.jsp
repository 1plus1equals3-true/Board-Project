...
if(word != null && !word.equals("")) {
	sql = " SELECT count(*) as cnt from board where boardtype = '2' and "+key+" like '%"+word+"%'";
}else {
	sql = "select count(*) as cnt from board WHERE boardtype = '2'";
}
...
try {
	...
	if(word != null && !word.equals("")) {
		sql = "SELECT b.*, m.name AS memname, COUNT(c.bidx) AS comment_count FROM board b LEFT JOIN member m ON b.uid = m.uid LEFT JOIN comment c ON b.idx = c.bidx WHERE boardtype='2' and b."+key+" LIKE '%"+word+"%' GROUP BY b.idx ORDER BY b.idx DESC limit "+offset+"," + scale;
	} else {
		sql = "SELECT b.*, COUNT(c.bidx) AS comment_count FROM board b LEFT JOIN comment c ON b.idx = c.bidx WHERE boardtype='2' GROUP BY b.idx ORDER BY b.idx DESC limit "+offset+"," + scale;
	}
