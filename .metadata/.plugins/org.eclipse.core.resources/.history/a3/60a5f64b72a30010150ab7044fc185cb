//========================전역===============================
let keepfile_length;
const file_total = 10; // 💡 최대 파일 업로드 제한 수 (10개)
//==========================================================
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (isConfirmed) {
                ...
                // 💡 [핵심] 기존 파일 삭제 시, 전역 변수 keepfile_length 감소
                keepfile_length--;
                ...
                if (fileItem) {
                    ...
                    console.log(`파일 삭제 및 hidden input 이동 완료. 남은 기존 파일 수: ${keepfile_length}`);
                }
            } else {
                // 사용자가 '취소'를 누른 경우
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleFileChange(event) {
        ...
        if (currentInput.files.length > 0) {
            ...
            // 1. 현재 파일 입력 필드 중 값이 있는(선택된) 개수를 계산합니다.
            let selectedNewFilesCount = 0;
            allInputs.forEach(input => {
                if (input.value) { // 값이 있는 input 개수
                    selectedNewFilesCount++;
                }
            });
            
            // 2. 총 파일 개수를 계산합니다. (기존 파일 수 + 새로 선택된 파일 수)
            const totalFiles = keepfile_length + selectedNewFilesCount;
            
            // 💡 [수정] 총 파일 개수가 file_total(10)을 초과하는지 확인 (11개째에 경고)
    ...
    function addNewFileInput(isInitial = false) {
        ...
        // 1. 현재 파일 입력 필드의 개수와 기존 파일 개수의 합이 10개와 같거나 크면 추가 방지
        const currentTotalInputs = keepfile_length + allInputs.length;
        
        // 💡 [수정] 파일 입력 필드가 10개째에 추가되는 것을 막아, 총 개수가 10개가 되도록 유지
        if (currentTotalInputs >= file_total) {
            // 초기화 시에는 경고를 띄울 필요가 없음
            if (!isInitial) { 
                alert(`더 이상 파일 입력 필드를 추가할 수 없습니다. (총 ${file_total}개까지 가능)`);
            }
            return; 
        }
